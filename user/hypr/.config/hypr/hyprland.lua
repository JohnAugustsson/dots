...
hl.on("hyprland.start", function()
    ...
    hl.exec_cmd("systemctl --user start keyd")
...
local function tree_contains(node, id)
    ...
    if node.id then return tostring(node.id) == tostring(id) end
    return tree_contains(node.a, id) or tree_contains(node.b, id)
end

local function tree_toggle_split_for_active(node, active_id)
    if not node or node.id or not active_id then return false end

    if tree_contains(node.a, active_id) then
        if tree_toggle_split_for_active(node.a, active_id) then return true end
        node.split = node.split == "vertical" and "horizontal" or "vertical"
        return true
    end

    if tree_contains(node.b, active_id) then
        if tree_toggle_split_for_active(node.b, active_id) then return true end
        node.split = node.split == "vertical" and "horizontal" or "vertical"
        return true
    end
...
local function tree_prune(node, present)
    ...
    if node.a and node.b then return node end
...
local function clamp_ratio(value)
    ...
    if value > 0.90 then return 0.90 end
    return value
end
...
local function tree_resize_active(ctx, node, active_id, dx, dy, area)
    ...
    if not active_in_a and not active_in_b then return false end
    ...
    if node.split == "vertical" then
        ...
        if active_in_a and tree_resize_active(ctx, node.a, active_id, dx, dy, a_area) then return true end
        if active_in_b and tree_resize_active(ctx, node.b, active_id, dx, dy, b_area) then return true end
...
local function tree_build_from_ids(ids, start_i, end_i, depth)
    if start_i > end_i then return nil end
    if start_i == end_i then return { id = ids[start_i] } end
...
local function is_direction_match(ax, ay, bx, by, dir)
    if dir == "left" then return bx < ax and math.abs(by - ay) <= math.max(1, math.abs(bx - ax) * 2) end
    if dir == "right" then return bx > ax and math.abs(by - ay) <= math.max(1, math.abs(bx - ax) * 2) end
    if dir == "up" then return by < ay and math.abs(bx - ax) <= math.max(1, math.abs(by - ay) * 2) end
    if dir == "down" then return by > ay and math.abs(bx - ax) <= math.max(1, math.abs(by - ay) * 2) end
    return false
end

local function triplewide_swap_active(active_id, dir)
    local active_box = active_id and triplewide_state.boxes[active_id]
    if not active_box then return false end
    ...
    if dir == "right" and not triplewide_state.right_id then
        ...
        for _, box in pairs(triplewide_state.boxes) do
            ...
            if box_right > max_right then max_right = box_right end
        end
    ...
    layout_msg = function(ctx, msg)
        ...
        if command == "resize" then
            ...
            if active_id and active_id ~= triplewide_state.right_id and dx and dy then
                tree_resize_active(ctx, triplewide_state.left_tree, active_id, tonumber(dx), tonumber(dy), ctx:split(ctx.area, "left", 0.5))
                reapply_triplewide(ctx)
            end
            return true
        end
...
local function bounded_workspace(dir)
    return function()
        ...
        if mon.name == "DP-1" then
            if dir == "next" then
                if ws == 1 then target = 2 elseif ws == 2 then target = 3 else target = 3 end
            else
                if ws == 3 then target = 2 elseif ws == 2 then target = 1 else target = 1 end
            end
        elseif mon.name == "DP-2" then
            if dir == "next" then
                if ws == 4 then target = 5 else target = 5 end
            else
                if ws == 5 then target = 4 else target = 4 end
            end
        end
...
hl.bind(mainMod .. " + Pause", hl.dsp.exec_cmd("command -v hyprshutdown >/dev/null 2>&1 && hyprshutdown || hyprctl dispatch exit"))
hl.bind(mainMod .. " + E", hl.dsp.exec_cmd(fileManager))
hl.bind(mainMod .. " + V", hl.dsp.window.float({ action = "toggle" }))
hl.bind(mainMod .. " + R", hl.dsp.exec_cmd(menu))
hl.bind(mainMod .. " + P", hl.dsp.window.pseudo())
hl.bind(mainMod .. " + END", hl.dsp.layout("togglesplit"))
hl.bind(mainMod .. " + U", hl.dsp.layout("togglesplit"))
hl.bind(mainMod .. " + ALT + END", hl.dsp.layout("togglesplit"))
hl.bind(mainMod .. " + CTRL + END", hl.dsp.layout("togglesplit"))
hl.bind(mainMod .. " + COMMA", hl.dsp.exec_cmd("hyprlock"))

-- HJKL tile management.
hl.bind(mainMod .. " + H", hypr_dispatch([[hl.dsp.focus({direction="left"})]]))
hl.bind(mainMod .. " + L", hypr_dispatch([[hl.dsp.focus({direction="right"})]]))
hl.bind(mainMod .. " + K", hypr_dispatch([[hl.dsp.focus({direction="up"})]]))
hl.bind(mainMod .. " + J", hypr_dispatch([[hl.dsp.focus({direction="down"})]]))
hl.bind(mainMod .. " + left", hypr_dispatch([[hl.dsp.focus({direction="left"})]]))
hl.bind(mainMod .. " + right", hypr_dispatch([[hl.dsp.focus({direction="right"})]]))
hl.bind(mainMod .. " + up", hypr_dispatch([[hl.dsp.focus({direction="up"})]]))
hl.bind(mainMod .. " + down", hypr_dispatch([[hl.dsp.focus({direction="down"})]]))

hl.bind(mainMod .. " + ALT + H", function() custom_triplewide_swap("left") end)
hl.bind(mainMod .. " + ALT + L", function() custom_triplewide_swap("right") end)
hl.bind(mainMod .. " + ALT + K", function() custom_triplewide_swap("up") end)
hl.bind(mainMod .. " + ALT + J", function() custom_triplewide_swap("down") end)
hl.bind(mainMod .. " + ALT + left", function() custom_triplewide_swap("left") end)
hl.bind(mainMod .. " + ALT + right", function() custom_triplewide_swap("right") end)
hl.bind(mainMod .. " + ALT + up", function() custom_triplewide_swap("up") end)
hl.bind(mainMod .. " + ALT + down", function() custom_triplewide_swap("down") end)

hl.bind(mainMod .. " + SHIFT + H", function() custom_triplewide_resize(-60, 0, -60, 0) end)
hl.bind(mainMod .. " + SHIFT + L", function() custom_triplewide_resize(60, 0, 60, 0) end)
hl.bind(mainMod .. " + SHIFT + K", function() custom_triplewide_resize(0, -60, 0, -60) end)
hl.bind(mainMod .. " + SHIFT + J", function() custom_triplewide_resize(0, 60, 0, 60) end)
...
hl.bind(mainMod .. " + CTRL + L", bounded_workspace("next"))
hl.bind(mainMod .. " + CTRL + H", bounded_workspace("prev"))
hl.bind(mainMod .. " + CTRL + right", bounded_workspace("next"))
hl.bind(mainMod .. " + CTRL + left", bounded_workspace("prev"))
hl.bind(mainMod .. " + CTRL + down", hl.dsp.focus({ monitor = "DP-1" }))
hl.bind(mainMod .. " + CTRL + J", hl.dsp.focus({ monitor = "DP-1" }))
hl.bind(mainMod .. " + CTRL + up", hl.dsp.focus({ monitor = "DP-2" }))
hl.bind(mainMod .. " + CTRL + K", hl.dsp.focus({ monitor = "DP-2" }))

-- Fake minimization
hl.bind(mainMod .. " + M", hl.dsp.window.move({ workspace = "special:hidden", silent = true }))
hl.bind(mainMod .. " + N", hl.dsp.workspace.toggle_special("hidden"))
hl.bind(mainMod .. " + TAB", hl.dsp.exec_cmd("kitty --class minimized-wins-popup --title minimized-wins-popup ~/.config/hypr/scripts/hypr-hidden-min.py --close-on-focus-lost"))
